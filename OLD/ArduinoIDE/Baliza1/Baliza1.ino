...
/*
      PRUEBA PARA EMISOR Y RECEPTOR CONECTO AL MISMO ARDUINO
      */
...
/*************************  CODIGO DE PRUEBA ********************************/
  ...
  while (true)
  {
    byte pin = digitalRead(PIN_RX_RF433Mhz);
...
return;
  ...
  if (ok == US_NO_DETECTADO)
        Serial.println("         NO_US");
  else if (ok)
       Serial.println("         ok..");
  else
       Serial.println("        failed");
