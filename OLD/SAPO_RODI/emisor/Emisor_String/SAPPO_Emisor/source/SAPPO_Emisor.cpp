...
void loop(void)
{
  float minimo, medida, media, maximo, filtroX, filtroY;
  ...
  for (i=0; i<1; i++)
  {
    ...
    //Si timeout = false implica que no hemos podido recuperar dos medidas válidas para triangular
    if (!timeout)
    {
...
void InicializarDatosBalizas(sBaliza &Balizas)
{
