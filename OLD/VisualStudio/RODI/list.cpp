...
// Insertar al inicio manteniendo una ventana de elementos máximo de num_elementos
template<typename T>
...
void List<T>::search(T data_)
{
	...
	while (temp) {
		if (temp->data == data_) {
			Serial.print("El dato se encuentra en la posición: ");
			Serial.println(cont);
			cont2++;
		}
		temp = temp->next;
...
T *List<T>::search_end()
{
	...
	if (!temp) return NULL; //Lista vacía
	while (temp->next);
	return temp;
}
