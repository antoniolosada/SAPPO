...
byte aTrigger[MAX_EMISORES_ULTRASONIDOS] = { TRIGER1, TRIGER2, TRIGER3, TRIGER4, TRIGER5, TRIGER7, TRIGER8, TRIGER6 };
byte aEcho[MAX_EMISORES_ULTRASONIDOS] = { ECHO1, ECHO2, ECHO3, ECHO4, ECHO5, ECHO7, ECHO8, ECHO6 };
...
PID2 Pid[2] = { PID2(&Input[IZQUIERDA], &Output[IZQUIERDA], &Setpoint[IZQUIERDA], 0.6, 0.3, 0.01, REVERSE), PID2(&Input[DERECHA], &Output[DERECHA], &Setpoint[DERECHA], 0.6, 0.3, 0.01, REVERSE) };
...
void setup(void)
{
	...
	for (int i = 0; i < 1; i++)
		for (int j = 0; j < 1; j++)
		{
			Odo[i].Contador[j].pulsos = Odo[i].Contador[j].Tiempo = Odo[i].Contador[j].Ultimo_ms = 0;
		}
	...
	//attachInterrupt(digitalPinToInterrupt(RUEDA_DER_ODO), intRueda_der, CHANGE);
	//attachInterrupt(digitalPinToInterrupt(RUEDA_IZQ_ODO), intRueda_izq, CHANGE);
...
void loop(void)
{
	...
	//Recuperar información de balizas de modo consecutivo
...
#define CICLO_ODO 20 //Se controla la odometría cada 8ms

ISR(TIMER2_COMPA_vect) {//timer1 interrupt 8kHz toggles pin 9

	if (contadorODO++ > CICLO_ODO)
	{
		//Incremento de tiempo en caso de que el tiempo grabado sea menor que el tiempo actual transcurrido desde el ultimo cambio
...
void PruebaODO()
{
	...
	//	InputGiro = 1;
	//else
	//	InputGiro = 1.0 * Odo[IZQUIERDA].pulsos_totales / Odo[DERECHA].pulsos_totales;
...
void LOG_ruedas(int log)
{
	...
	if (log == 1)
	{
		...
		if (Odo[DERECHA].pulsos_totales != pulsos_ant)
		{
			...
			Serial.print(" - ");
			Serial.println(1.0 * Odo[IZQUIERDA].pulsos_totales/ Odo[DERECHA].pulsos_totales);
			pulsos_ant++;
		}
	}
	if (log == 2)
...
bool RecuperarMedidasBalizas(int algoritmo, bool salida)
{
	...
	timeout = RecuperarMedidaMultiple(EMISION_CONJUNTA);
...
void EnviarPulsoUltrasonidos(int emisor)
{
	...
	{
		digitalWrite(aTrigger[emisor], LOW); /* Por cuestión de estabilización del sensor*/
		delayMicroseconds(5);
		digitalWrite(aTrigger[emisor], HIGH); /* envío del pulso ultrasónico*/
		delayMicroseconds(15);
		digitalWrite(aTrigger[emisor], LOW); /* envío del pulso ultrasónico*/
	}
	else
	{
		for (int i = 0; i<MAX_EMISORES_ULTRASONIDOS; i++)
...
void EjecutarComando()
{
	...
	{
		if (comando.length() != 6) return;
		contador = TIMER2_OVF;
		PararMotores = false;
	}
	else if (comando.substring(0, 2) == "kl")
	{
		if (comando.length() != 10) return;
...
/* 
	* Si estoy leyendo el valor de pulso X y tengo una señal <> X
		1. Si no hay activa transición, la activo
		2. Si hay activa transición, sigue contando el tiempo de transición
    * Si estoy leyendo el valor de pulso X y tengo una señal == X
		1. Si no hay activa transición, no hago nada sigo leyendo X
		2. Si hay activa una transcion

*/
sTransicion uno[2] = { { 0, 0, no_activa },{ 0, 0, no_activa } }, cero[2] = { { 0, 0, no_activa },{ 0, 0, no_activa } };
...
void ContarPulsoRueda(int rueda, int lecturaAct)
{
	...
	if (tmp_uno->estado == pendiente)
	{
		...
		if (tiempo > TIEMPO_PULSO)
		{
			...
			Odo[rueda].Contador[lecturaAct].Tiempo = tiempo_ms - Odo[rueda].Contador[lecturaAct].Ultimo_ms;
		...
		//Siempre que no se detecte una interferencia se inicializa el pulso contrario
		if (ini_pulso)
		{
